#!/bin/bash
# iterate_v3.sh — Backbone→transient→backbone iteration for Blue Earth River v3.x.
#
# 1-reservoir till+tile architecture:
#   - backbone calibrates snow/FGI only (PDD, FGI threshold)
#   - transient calibrates per-decade: κ_till, Hmax_till, et_scale
#   - iteration carries per-decade κ_till back into backbone for updated snow/FGI fit
#
# Usage: bash iterate_v3.sh <existing_backbone_run_dir> <current_minor_ver> [max_iters]
# e.g.:  bash iterate_v3.sh backbone_runs/2026-06-19_XXXXXX_backbone_v3.0 0 4
#
...
EXISTING_BB_DIR="${1:?Usage: bash iterate_v3.sh <backbone_run_dir> <minor_ver> [max_iters]}"
MINOR_VER="${2:?}"
MAX_ITERS="${3:-4}"
...
for dec in decades_order:
    ...
    rows_out.append((
        ...
        best.get('log__Hmax_till', float('nan')),
        best.get('et_scale', float('nan')),
        len(valid),
    ))

print(f"\n  {'Decade':<12} {'KGE_logKGE':>11} {'log_κ_till':>11} {'log_Hmax':>9} {'et_scale':>9} {'evals':>6}")
print(f"  {'-'*12} {'-'*11} {'-'*11} {'-'*9} {'-'*9} {'-'*6}")
kges = []
for dec, kge, lk, lh, es, n in rows_out:
    if kge is not None:
        print(f"  {dec:<12} {kge:>11.4f} {lk:>11.4f} {lh:>9.4f} {es:>9.4f} {n:>6}")
        kges.append(kge)
    else:
        print(f"  {dec:<12} {'—':>11} {'—':>11} {'—':>9} {'—':>9} {n:>6}")
if kges:
...
for ITER in $(seq 1 $MAX_ITERS); do
    ...
    $PYTHON make_transient_params_v3.0.py --backbone "$CURRENT_BB_EVAL" --version "$VER"

    # 3. Run transient
    log "Starting transient v${VER}..."
    bash run_transient.sh "transient_v${VER}" driver_transient_v3.0.py "_v${VER}" \
        2>&1 | tee "/tmp/transient_be_v${VER}.log"

    # 4. Transient summary
    transient_summary "$VER"

    # 5. Extract κ_till and Hmax_till
    $PYTHON extract_ktill_best.py --desc "transient_v${VER}" --save "ktill_v${VER}.yml"
    ...
    bash run_backbone.sh "backbone_v${NEXT_VER}" "params_backbone_v${NEXT_VER}.yml" \
        driver_backbone_v3.0.py 2>&1 | tee "/tmp/backbone_be_v${NEXT_VER}.log"
