#!/usr/bin/env python3
"""
Generate params_transient_v6.0_tau_mp.yml in each calibrated decade dir.

v6 per-decade experiment #1: τ_multipath per decade (tile-expansion hypothesis).
  - All other A3 hydraulic + snow params fixed at v6.1 best-fit (static).
  - Only log__multipath_timescale_till active per decade.
  - Tests monotonic decline of τ_mp over decades (tile density grew → drainage sped up).
"""

import yaml
from pathlib import Path

# v6.1 global best-fit (KGE = 0.7751)
V61 = {
    'PDD_melt_factor':                 6.0457,
    'log__fdd_threshold':              1.1861,
    ...
    return {
        ...
        'parameters': {
            'log__multipath_timescale_till': {
                'description': 'log10 multipath τ — only per-decade active (tile-expansion test)',
                'lower': 0.0, 'upper': 2.0,
            ...
            'log__fdd_threshold': {
                'description': 'log10 FGI threshold — v6.1 fixed',
                'lower': 0.0, 'upper': 4.0,
            ...
            'log__multipath_threshold_till': {
                'description': 'log10 multipath H_thr — v6.1 fixed (global)',
                'lower': 0.5, 'upper': 3.0,
...
def main():
    ...
    for d in DECADE_DIRS:
        ...
        with open(out, 'w') as f:
            f.write(f"# {d.name}: v6.0 per-decade τ_multipath calibration.\n")
            f.write(f"# Active (1): log__multipath_timescale_till.\n")
            f.write(f"# Fixed: all snow + matrix τ + H_thr at v6.1 best-fit.\n\n")
