#!/bin/bash
# iterate_v1.sh — Full backbone→transient→backbone iteration loop for Blue Earth River v1.x.
#
...
#   9. Repeat
#
# Usage: bash iterate_v1.sh <existing_backbone_run_dir> <current_minor_ver> [max_iters]
# e.g.:  bash iterate_v1.sh backbone_runs/2026-06-19_120000_backbone_v1.0 0 6
#
...
EXISTING_BB_DIR="${1:?Usage: bash iterate_v1.sh <backbone_run_dir> <minor_ver> [max_iters]}"
MINOR_VER="${2:?}"
MAX_ITERS="${3:-6}"
...
if kges:
    ...
    print(kges[-1])   # sentinel: last line is last-decade KGE (not used; mean printed above)
PYEOF
}
...
for ITER in $(seq 1 $MAX_ITERS); do
    ...
    $PYTHON make_transient_params_v1.0.py --backbone "$CURRENT_BB_EVAL" --version "$VER"

    # 3. Run transient
    log "Starting transient v${VER}..."
    bash run_transient.sh "transient_v${VER}" driver_transient_v1.0.py "_v${VER}" \
    ...
    bash run_backbone.sh "backbone_v${NEXT_VER}" "params_backbone_v${NEXT_VER}.yml" \
        driver_backbone_v1.0.py 2>&1 | tee "/tmp/backbone_be_v${NEXT_VER}.log"
