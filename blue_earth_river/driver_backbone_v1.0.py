#!/usr/bin/env python3
"""
Joint backbone Dakota driver v1.0: 2-reservoir soil/till architecture (Blue Earth River).

Active params (3): till recession (1) + snow/FGI (2), shared across decades.
...
2-reservoir structure (no PdC leakance, no Wonewoc threshold):
  soil (b=2, Dupuit-Forchheimer, Hmax=∞ in backbone) →
  till (b=1 linear, all drainage to stream)

Per-decade log__recession_coeff_soil values embedded by make_backbone_next_v1.py.
Absent in v1.0 first run; present from v1.1 onward.
"""
...
RESERVOIR_ORDER = _driver.get('reservoir_order', ['soil', 'till'])
