#!/bin/bash
# iterate_v2.sh — Full backbone→transient→backbone iteration loop for Blue Earth River v2.x.
#
# 3-reservoir architecture: soil (b=2) → shallow till (b=1) → deep till (b=1).
#
...
#   6. Convergence check (stop if ΔKGE < tol, ending on a transient)
#   7. Generate next backbone params
#   8. Run next backbone
#   9. Repeat
#
# Usage: bash iterate_v2.sh <existing_backbone_run_dir> <current_minor_ver> [max_iters]
# e.g.:  bash iterate_v2.sh backbone_runs/2026-06-19_120000_backbone_v2.0 0 6
#
...
EXISTING_BB_DIR="${1:?Usage: bash iterate_v2.sh <backbone_run_dir> <minor_ver> [max_iters]}"
MINOR_VER="${2:?}"
MAX_ITERS="${3:-6}"
...
params = [
    ...
    'PDD_melt_factor', 'log__fdd_threshold',
...
print(f"\n  {'Decade':<12} {'KGE_logKGE':>11} {'log_κ_soil':>11} "
...
for ITER in $(seq 1 $MAX_ITERS); do
    ...
    $PYTHON make_transient_params_v2.0.py --backbone "$CURRENT_BB_EVAL" --version "$VER"

    # 3. Run transient
    log "Starting transient v${VER}..."
    bash run_transient.sh "transient_v${VER}" driver_transient_v2.0.py "_v${VER}" \
    ...
    # Stop after transient if converged (always end on a transient).
    if [[ "$ITER" -gt 1 ]]; then
    ...
    bash run_backbone.sh "backbone_v${NEXT_VER}" "params_backbone_v${NEXT_VER}.yml" \
        driver_backbone_v2.0.py 2>&1 | tee "/tmp/backbone_be_v${NEXT_VER}.log"

    # Find the new backbone run dir
