...
v5.0 architecture (Blue Earth River):
  - 1 reservoir: till+tile (b calibrated per-decade, NO Hmax)
  - backbone fixed: snow/FGI only (PDD_melt_factor, log__fdd_threshold)
  - 3 active per-decade params: log__recession_coeff_till, recession_b_till, et_scale

Physical motivation: b_till per-decade tracks changing till+tile drainage nonlinearity
as tile density expanded. Pre-tile b≈2 (Dupuit-Forchheimer); tile-dominated system
may shift b toward 1 (linear pipe flow). No Hmax — b absorbs the nonlinear shape.

Usage:
    python make_transient_params_v5.0.py --backbone backbone_runs/<run_dir>/evaluations.dat
    python make_transient_params_v5.0.py --backbone ... --version 5.1
"""
    ...
    return {
        ...
        'parameters': {
            ...
            'log__recession_coeff_till': {
                'description': 'log10 till+tile recession coefficient κ [days] — per-decade',
                'lower': 1.0, 'upper': 4.5, 'initial': 2.3646, 'fixed': 2.3646,
                'active': True,
            },
            'recession_b_till': {
                'description': 'till+tile recession exponent b — per-decade (pre-tile≈2, tile-era→lower)',
...
def main():
    ...
    parser.add_argument('--version', default='5.0',
                        help='Version label for output filenames (default: 5.0)')
    ...
    for decade_dir in DECADE_DIRS:
        ...
        with open(out_path, 'w') as f:
            ...
            f.write(f"# Backbone fixed (backbone_v{ver}); 3 active: "
                    f"log__recession_coeff_till, recession_b_till, et_scale.\n")
            f.write(f"# No Hmax; b_till free (pre-tile≈2, tile-era→lower). 1-reservoir till+tile.\n")
            if first_decade:
