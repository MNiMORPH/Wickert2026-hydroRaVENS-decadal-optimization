#!/usr/bin/env python3
"""
Generate params_transient_v1.0.yml (and later) for every calibrated decade directory.

v1.0 architecture (Blue Earth River):
  - 2 reservoirs: soil (b=2 Dupuit-Forchheimer, Hmax calibrated) → till (b=1 linear)
  - backbone fixed: till recession (1) + snow/FGI (2), from backbone_v1.x evaluations.dat
  - 4 active per-decade params: log__recession_coeff_soil, f_exfiltration_soil,
                                log__Hmax_soil, et_scale

Usage:
    python make_transient_params_v1.0.py --backbone backbone_runs/<run_dir>/evaluations.dat
    python make_transient_params_v1.0.py --backbone ... --version 1.1
"""
...
def make_params(decade_dir, backbone, start, end, is_first, ver):
    return {
        ...
        'driver': {
            'config_template':       'blue_earth_config_v1.yml',
            'metric':                'KGE_logKGE',
            'spin_up_cycles':        1 if is_first else 0,
            'routing_N':             2,
            'n_reservoirs':          2,
            'reservoir_order':       ['soil', 'till'],
            'decade_start':          start,
            'decade_end':            end,
        ...
        'parameters': {
            ...
            'f_exfiltration_soil': {
                'description': 'fraction of soil drainage to stream (remainder recharges till) — per-decade',
            ...
            'log__recession_coeff_till': {
                'description': f'log10 till recession coefficient κ — backbone_v{ver} fixed',
                'lower': 1.0, 'upper': 5.0,
                'initial': backbone['log__recession_coeff_till'],
                'fixed':   backbone['log__recession_coeff_till'],
                'active': False,
            },
            'PDD_melt_factor': {
            ...
            'log__H0_till': {
                'description': 'log10 initial till H [mm] — set by IC chain',
...
def main():
    ...
    parser.add_argument('--version', default='1.0',
                        help='Version label for output filenames (default: 1.0)')
    ...
    for decade_dir in DECADE_DIRS:
        ...
        with open(out_path, 'w') as f:
            ...
            f.write(f"# b_soil=2 fixed; 2-reservoir soil/till architecture.\n")
            if first_decade:
