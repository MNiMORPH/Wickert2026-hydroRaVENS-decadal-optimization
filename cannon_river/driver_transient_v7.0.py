#!/usr/bin/env python3
"""
Per-decade transient Dakota driver v7.0: Cannon River, multipath on soil reservoir.

Per-decade test #1: τ_multipath_soil varies; all other soil + geology + snow
params fixed at v7.0 (or v7.1) backbone best-fit.

Active per decade (1): log__multipath_timescale_soil.
Fixed: PDD, log__fdd_threshold (if FGI on), log__recession_coeff_soil,
       log__multipath_threshold_soil, f_exfiltration_soil, et_scale,
       and intermediate + deep reservoir geology.
ICs chained via log__H0_{l}.
"""
...
try:
    ...
    result = run_and_score(
        ...
        multipath_calibrated           = 1,           # 1 multipath param per decade
