#!/usr/bin/env python3
"""
Joint backbone Dakota driver v6.0: soil-reservoir tile-drain architecture.

Active params (7): geologic (5) + snow (2), shared across decades.
...
Hmax_soil = ∞ (v6.0 baseline); becomes active in v6.1+.

Tile drainage is no longer modelled by a separate sub-reservoir.  Instead:
  - τ_recession_soil (via log__recession_coeff_soil) captures drainage efficiency.
  - Hmax_soil (when < ∞) caps soil storage at the effective drain depth.

Per-decade log__recession_coeff_soil values (from transient) are embedded in
driver.per_decade_k_soil and applied to each decade window.  Falls back to
the global fixed value when the dict is absent (v6.0 first run).
"""
