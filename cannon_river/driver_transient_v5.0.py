#!/usr/bin/env python3
"""
Per-decade transient Dakota driver v5.0: explicit tile-drain architecture.

Active params (4): log__recession_coeff_soil, f_tile_soil,
                   f_exfiltration_soil, et_scale.
Fixed params: backbone (geology + snow + tau_tile), b_soil=2, H0 (chained).
...
Initial conditions:
  ...
  - If spin_up_cycles == 0 (chained decades): H0 values used directly as
    initial_states, no spin-up (transient chain preserved).
"""
...
def _h0_states():
    """Read log__H0_{label} fixed values as initial reservoir states [mm].
    Returns dict suitable for initial_states or post_spinup_states, or None."""
...
try:
    ...
    if SPIN_UP_CYCLES == 0 and _h0 is not None:
        # Chained mode: use H0 directly as initial_states, no spin-up.
        _initial_states     = _h0
        _post_spinup_states = None
    else:
        # First-decade mode: run clean pre-decade spin-up (1893-1930); its
        # end states become the decade ICs directly.
