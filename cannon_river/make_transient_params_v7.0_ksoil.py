#!/usr/bin/env python3
"""
Generate params_transient_v7.0_ksoil.yml in each calibrated decade dir.

Cannon v7 per-decade experiment #3: matrix τ (log__recession_coeff_soil) per decade.

Cannon v6 transient showed log_k_soil ranged 1.64–1.99 across decades — the
largest swing among non-Hmax soil parameters. Matrix τ controls the *slow*
drainage path; multipath τ controls the *fast* path. These probe different
hydrograph timescales, so per-decade matrix τ may carry independent signal
that τ_mp per-decade (+0.015) did not.

Active per decade (1): log__recession_coeff_soil.
Fixed at v7.0 best-fit: all other params (PDD, FGI, multipath H_thr, τ_mp,
       f_exfil_soil, et_scale, intermediate, deep).
"""
...
def main():
    ...
    for d in DECADE_DIRS:
        ...
        with open(out, 'w') as f:
            f.write(f"# {d.name}: Cannon v7.0 per-decade log_κ_soil (matrix τ) calibration.\n")
            f.write(f"# Active (1): log__recession_coeff_soil.\n")
            f.write(f"# Fixed: snow + multipath (H_thr, τ_mp) + f_exfil + et_scale at v7.0 best-fit.\n\n")
