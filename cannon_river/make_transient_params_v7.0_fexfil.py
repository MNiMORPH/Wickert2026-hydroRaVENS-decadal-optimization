#!/usr/bin/env python3
"""
Generate params_transient_v7.0_fexfil.yml in each calibrated decade dir.

Cannon v7 per-decade experiment #4: f_exfiltration_soil per decade.

Tests whether the matrix-outflow partition (stream vs. intermediate reservoir)
varies decadally. v6 transient saw f_exfil_soil swing 0.69–0.83. May be
correlated with the strong κ_soil per-decade signal (+0.036 KGE); this is
the independent test of partition-vs-rate.

Active per decade (1): f_exfiltration_soil.
...
def make_params(start, end, is_first):
    return {
        ...
        'parameters': {
            ...
            'f_exfiltration_soil': {
                'description': 'soil-reservoir matrix outflow fraction — per-decade active (partition test)',
                'lower': 0.0, 'upper': 1.0,
...
def main():
    ...
    for d in DECADE_DIRS:
        ...
        out = d / 'params_transient_v7.0_fexfil.yml'
        with open(out, 'w') as f:
            f.write(f"# {d.name}: Cannon v7.0 per-decade f_exfil_soil calibration.\n")
            f.write(f"# Active (1): f_exfiltration_soil.\n")
            f.write(f"# Fixed: snow + multipath + matrix τ + et_scale at v7.0 best-fit.\n\n")
