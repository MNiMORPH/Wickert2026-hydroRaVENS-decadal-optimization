#!/usr/bin/env python3
"""
Joint backbone Dakota driver: scores across all full-coverage decades simultaneously.

Active params (7): geologic + snow backbone shared across decades.
Fixed params: per-decade soil/ET held at mid-range for backbone search.

Each decade is scored independently using analytical steady-state ICs +
spin_up_cycles=1 (no chaining during backbone calibration). This avoids
parameter-dependent ICs contaminating the backbone signal.

Returns mean KGE_logKGE across all valid decades (those meeting min_obs).
"""
...
# Pre-screen decades for sufficient discharge coverage.
...
def _f_discharge():
    """Aligned f_to_discharge list: None for reservoirs without an active/present
    f_exfiltration param, so calibration.py correctly skips those reservoirs."""
    vals = []
