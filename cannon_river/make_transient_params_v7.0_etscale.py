#!/usr/bin/env python3
"""
Generate params_transient_v7.0_etscale.yml in each calibrated decade dir.

Cannon v7 per-decade experiment #2: et_scale per decade.
Mirrors the BLR D3 experiment but in Cannon's 3-reservoir + multipath context.

Cannon v6 transient showed et_scale ranged 0.73–0.92 across decades.
Tests whether allowing et_scale per-decade closes the 0.012 gap
between v7 static (KGE 0.710) and v6 transient (KGE 0.737).

Active per decade (1): et_scale.
Fixed at v7.0 best-fit: all other params (PDD, FGI, soil matrix τ,
       multipath H_thr, τ_mp, f_exfil_soil, intermediate, deep).
"""
...
def make_params(start, end, is_first):
    return {
        ...
        'parameters': {
            'et_scale': {
                'description': 'Thornthwaite ET multiplier — only per-decade active (ET bias test)',
                'lower': 0.3, 'upper': 2.0,
...
def main():
    ...
    for d in DECADE_DIRS:
        ...
        out = d / 'params_transient_v7.0_etscale.yml'
        with open(out, 'w') as f:
            f.write(f"# {d.name}: Cannon v7.0 per-decade et_scale calibration.\n")
            f.write(f"# Active (1): et_scale.\n")
            f.write(f"# Fixed: snow + soil multipath + matrix τ + f_exfil at v7.0 best-fit.\n\n")
