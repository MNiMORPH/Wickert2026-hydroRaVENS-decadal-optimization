#!/usr/bin/env python3
"""
Generate params_transient_v7.0_ksoil_fexfil.yml in each calibrated decade dir.

Cannon v7 per-decade experiment #5: JOINT log_κ_soil + f_exfil_soil per decade.

Multi-layer tile-expansion hypothesis: tile expansion in Cannon's till+loess
surface should both shorten matrix τ (faster drainage) and raise f_exfil
(more flux to stream, less to PdC dolomite). Tested individually:
- κ_soil per-decade alone: +0.037 KGE, slope -0.082/decade (predicted)
- f_exfil per-decade alone: -0.016 KGE, slope +0.021/decade (predicted)
  (κ↔f_exfil coupling: f_exfil compensates poorly for wrong fixed κ_soil)

Joint test: reveals whether both trends survive and whether KGE matches/
exceeds v6.3 transient (0.737).

Active per decade (2): log__recession_coeff_soil, f_exfiltration_soil.
Fixed at v7.0 best-fit: all other params.
"""
    ...
    return {
        ...
        'parameters': {
            ...
            'log__recession_coeff_soil': {
                'description': 'log10 soil matrix τ — per-decade active (slow-path test)',
                'lower': 0.5, 'upper': 4.0,
            ...
            'log__multipath_timescale_soil': {
                'description': 'log10 multipath τ — v7.0 fixed (global)',
                'lower': 0.0, 'upper': 2.0,
            ...
            'f_exfiltration_soil': {
                'description': 'soil-reservoir matrix outflow fraction — per-decade (joint test)',
                'lower': 0.0, 'upper': 1.0,
...
def main():
    ...
    for d in DECADE_DIRS:
        ...
        out = d / 'params_transient_v7.0_ksoil_fexfil.yml'
        with open(out, 'w') as f:
            f.write(f"# {d.name}: Cannon v7.0 joint per-decade log_κ_soil + f_exfil_soil calibration.\n")
            f.write(f"# Active (2): log__recession_coeff_soil, f_exfiltration_soil.\n")
            f.write(f"# Fixed: snow + multipath + et_scale at v7.0 best-fit.\n\n")
