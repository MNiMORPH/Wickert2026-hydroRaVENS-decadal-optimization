#!/usr/bin/env python3
"""
Generate params_transient_v6.0.yml for every calibrated decade directory.
...
Usage:
    ...
    python make_transient_params_v6.0.py --backbone ... --version 6.1
"""
...
BACKBONE_DEFAULTS = {
    'log__recession_coeff_intermediate': 2.5,
    'log__recession_coeff_deep':         4.0,
    'log__leakance_R_intermediate':      2.5,
    'f_exfiltration_deep':               0.40,
    'log__H_threshold_deep':             2.0,
    'PDD_melt_factor':                   5.5,
...
def make_params(decade_dir, backbone, start, end, is_first, ver):
    return {
        ...
        'parameters': {
            ...
            'f_exfiltration_soil': {
                'description': 'fraction of soil drainage to stream — per-decade',
            ...
            'log__recession_coeff_intermediate': {
                'description': f'log10 PdC recession coefficient κ — backbone_v{ver} fixed',
                'lower': 1.0, 'upper': 5.0,
            ...
            'log__recession_coeff_deep': {
                'description': f'log10 Wonewoc recession coefficient κ — backbone_v{ver} fixed',
                'lower': 3.0, 'upper': 5.5,
            ...
            'log__leakance_R_intermediate': {
                'description': f'log10 St. Lawrence shale leakance resistance — backbone_v{ver} fixed',
                'lower': 0.5, 'upper': 5.0,
            ...
            'f_exfiltration_deep': {
                'description': f'Wonewoc fraction to stream — backbone_v{ver} fixed',
            ...
            'log__H_threshold_deep': {
                'description': f'log10 Wonewoc spring sill [mm] — backbone_v{ver} fixed',
                'lower': 0.0, 'upper': 3.5,
...
def main():
    ...
    parser.add_argument('--version', default='6.0',
                        help='Version label for output filenames (default: 6.0)')
