...
def _et_scale(row):
    if 'et_scale' not in (_PARAMS or {}):
        return None
    return _get(row, 'et_scale')
...
def make_plot(result, params_row, save_path):
    ...
    if _is_active('et_scale'):
        param_lines += f',  $s_{{ET}}$ = {_get(params_row, "et_scale"):.3f}'
    ...
    fig.suptitle('MNiShed – Wild Rice River at Twin Valley best-fit calibration', fontsize=13)
...
if __name__ == '__main__':
    ...
    if _is_active('et_scale'):
        print(f'  et_scale         = {_get(best, "et_scale"):.4f}')
    if _is_active('et_alpha'):
