#!/usr/bin/env python3
"""
Joint backbone Dakota driver v6.0 — 3-reservoir Root River variant.

Adapted from Cannon driver_backbone_v6.0.py. Differences:
  - et_scale is calibrated (active, constant across decades) instead of
    being closed by enforce_water_balance='global'.
  - Soil reservoir is loess + topsoil (not tile-drained glacial drift),
    so b_soil = 1 (linear) by default rather than 2 (Dupuit-Forchheimer).
  - log__recession_coeff_soil is active (Cannon had it per-decade in transient).

Active params (Root v6.0): geologic (7) + snow (1) + ET (1) = 9 total.
Cannon's per_decade_k_soil mechanism is preserved (falls through to fixed
when absent), but not used in Root's first run.
"""
...
# Per-decade log__recession_coeff_soil values embedded by make_backbone_next_v6.py.
# Absent in v6.0 first run; present from v6.1 onward.
...
try:
    ...
    _f_dis      = _f_discharge()
    _hmax       = _Hmax()   # None in v6.0 (uses config ∞)

    scores = []
    for _dec in _valid_decades:
        # Per-decade τ_soil: from transient summary (v6.1+) or fixed (v6.0).
